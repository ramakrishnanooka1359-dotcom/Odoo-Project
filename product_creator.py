...
def get_uom_id(logical_name):
    ...
    def create_product_template(product_name, uom_name):
        ...
        time.sleep(1)
        set_variant_prices_stock_and_internal_ref(template_id, product_name)

        return template_id

    # =================================================
    # Variant pricing + stock + internal reference
    # =================================================

    def set_variant_prices_stock_and_internal_ref(template_id, product_name):
        ...
        for variant in variants:
            ...
            # Only one Pack Size per variant
            ...
            # Variant price_extra
            # -----------------------------
            extra_price = PRICE_MAP[pname].get(size_key, 0)

            odoo.call(
                "product.template.attribute.value",
                "write",
                [[ptavs[0]["id"]], {"price_extra": extra_price}]
            )

            # -----------------------------
            # Internal Reference (SKU)
            # -----------------------------
            internal_ref = f"{pname_code}-{size_key}"

            odoo.call(
                "product.product",
                "write",
                [[variant["id"]], {"default_code": internal_ref}]
            )
            ...
            print(f"✅ {product_name} | {size_name} | SKU = {internal_ref}")
